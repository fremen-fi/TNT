...
func (n *AudioNormalizer) buildBrightnessReduceFilterForLossy(strength int) string {
	n.logToFile(n.logFile, "Reducing brightness because of a lossy codec.")
...
func (n *AudioNormalizer) buildPCMBrightnessReductionFilter(strength int) string {
	n.logToFile(n.logFile, "Reducing brightness because of a dynamics processor.")
	n.logToFile(n.logFile, fmt.Sprintf("Strength set to %d", strength))

	filter := audio.ReduceBrightnessPCM(strength)
	n.logToFile(n.logFile, fmt.Sprintf("reduced brightness before a dynamics processor: %s", filter))
